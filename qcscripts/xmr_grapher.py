...
def xmr_plot(datadict, tolerance, tolerancetype, units, analyte, filename):
    ...
    for level in sorted(datadict.keys()):
        ...
        ratiolabel = []
        if tolerancetype == 'add':
            if analyte in strangeanalytes:
                for a in tolerance[1:-1]:
                    ratiolabel.append('~(' + str(int(((n_value+a)/n_value)*100))+'%)')
            else:
                for a in tolerance[1:-1]:
                    ratiolabel.append(str(int(((n_value+a)/n_value)*100))+'%')
        elif tolerancetype == 'multi':
    ...
    #plot the measured data
        ...
        ax2.axis([0,len(xdata)+1,yraw_labels[0],yraw_labels[-1]])
        ...
        #plot the data
        ydatalen = map(lambda x: len(x),ydata)
        maxlength = np.max(ydatalen)
        normydata = []
        for item in ydata:
            if maxlength > len(item):
                delta = maxlength-len(item)
                v = 0
                while v < delta:
                    item = np.append(item,np.nan)
                    v += 1
        ...
        if yraw_labels[-1] - yraw_labels[0] < 1:
            axes[row,1].text(0.1, 1.01*(yraw_labels[0]), 'Mean: ' + str(round(m_mean,3))+units, fontsize = 'x-small', color = 'r')
        else:
            axes[row,1].text(0.1, 1.05*(yraw_labels[0]), 'Mean: ' + str(round(m_mean,3))+units, fontsize = 'x-small', color = 'r')
        ...
        outputdict[level] = {'agg':[str(round(m_mean,2))+units,
                                    ...
                                    str(round(x_lcl,3))+units, str(round(x_ucl,3))+units],
                             'rdata':rangeoutputlist,
                             'ragg':[str(round(r_mean,2))+units,
                                     str(round(np.std(rd),3))+units,
                                     str(round((np.std(rd)/float(r_mean))*100,3))+'%',
                                     str(round(r_ucl,3))+units],
                             'n_value':str(n_value)+units}
